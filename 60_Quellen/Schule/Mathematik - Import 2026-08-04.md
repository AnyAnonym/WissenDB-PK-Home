# Mathematik – Import 2026-08-04

## Metadaten

- Quellentyp: Schulmaterialien und Unterrichtsunterlagen
- Importkennung: `2026-08-04_094550`
- Erfasst: 2026-08-04
- Zuletzt geändert: 2026-08-04
- Status: importiert; eine vermutlich sensible Datei wurde nicht verarbeitet

## Kurzbeschreibung

Der mathematische Teil des Importlaufs umfasst Arbeits- und Lösungsblätter,
Aufgabensammlungen, historische Prüfungsthemen und eine offizielle schulische
Formelsammlung.

## Themenumfang

- Zahlen, Mengen und Intervalle
- Terme, Brüche, Potenzen und Wurzeln
- Proportionen und Prozentrechnung
- Gleichungen und lineare Gleichungssysteme
- Funktionen und lineare Funktionen
- Geometrie und Trigonometrie
- Vektorrechnung
- historische Prüfungsvorbereitung für 2025/2026

## Qualität und Besonderheiten

- `Formelsammlung-Mathematik.pdf` und `srdp_am_ma_formelsammlung.pdf` sind
  bitidentische Fassungen der offiziellen schulischen Formelsammlung. Beide
  Originale bleiben aufbewahrungswürdig.
- `LINKS .odt` enthält interaktive Trigonometrie-Verweise. Zwei
  Westermann-Seiten wurden am 2026-08-04 geprüft; drei nicht verifizierbare
  Verweise wurden nicht als verlässliche Quellen übernommen.
- Ein Elternbrief mit einem vermutlich vertraulichen Bestell- oder Zugangscode
  wurde nicht verarbeitet. Sein aktueller Verbleib ist im aktiven Bestand nicht
  dokumentiert; er befindet sich derzeit nicht in `00_Eingang`.

## Abgeleitete Einträge

Die dauerhaft relevanten Inhalte wurden in thematische Artikel unter
`10_Wissen/Lernen/Mathematik` zusammengeführt. Die historischen Prüfungsthemen
bleiben als Bestandteil des Quellenumfangs dokumentiert, werden jedoch nicht als
eigener Wissenseintrag geführt.
